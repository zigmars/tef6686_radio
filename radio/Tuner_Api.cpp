...
/* FM1, FM2, FM3, LW, MW, SW */
const Range PROGMEM FreqBoundConfig[MaxBandNum] =
#if (AreaSelect==Radio_CHN)   /*China */
...
void Radio_SetFreq(uint8_t mode, uint8_t Band, uint16_t Freq) {
    ...
    Range freqRange;
    memcpy_P(&freqRange, &FreqBoundConfig[Radio_CurrentBand], sizeof(Range));
    if ((Freq > freqRange.Max) || (Freq < freqRange.Min)) Freq = freqRange.Min;
    if (Band >= MaxBandNum) return;
    ...
    if (Is_Radio_Atomic2||Is_Radio_Lithio) {
        ...
        devTEF668x_Radio_Tune_To(TEF665X_Is_FM_Freq(Freq),
                                 (uint16_t)A2Mode, Freq);
    }
}
/*====================================================
...
void Radio_ChangeFreqOneStep(uint8_t UpDown ) {
	int step = Radio_GetFreqStep(Radio_CurrentBand);
    Range freqRange;
    memcpy_P(&freqRange, &FreqBoundConfig[Radio_CurrentBand], sizeof(Range));
	if (UpDown == 1) {	/*increase one step*/
		Radio_CurrentFreq += step;
		/*frequency baundary check	*/
		if (Radio_CurrentFreq > freqRange.Max) {
			Radio_CurrentFreq = freqRange.Min;
		}
	} else {      /*decrease one step*/
		Radio_CurrentFreq -= step;
		/*frequency baundary check*/
		if (Radio_CurrentFreq < freqRange.Min) {
			Radio_CurrentFreq = freqRange.Max;
		}
	}
...
uint32_t Radio_GetFreqStep(uint8_t band) {
    Radio_AreaConfigDef conf;
    memcpy_P(&conf, &Radio_AreaConfig, sizeof(Radio_AreaConfigDef));
	return (band<=FM3_BAND) ? conf.FM_ManualSeekStep
                            : conf.AM_ManualSeekStep;
}
/*-----------------------------------------------------------------------
...
uint16_t Radio_Get_Level(uint8_t fm) {
	...
	if ((Is_Radio_Atomic2 || Is_Radio_Lithio) &&
		(devTEF668x_Radio_Get_Quality_Level(fm,&status,&level) == 1)) {
		return level;
	}
	return -255;
}
...
static int Radio_Get_Data(uint8_t fm, uint8_t *usn, uint8_t *wam, uint16_t *offset) {
	if ((Is_Radio_Atomic2||Is_Radio_Lithio) &&
		(devTEF668x_Radio_Get_Quality_Data (fm,usn,wam,offset) == 1)) {
		return 1;
	}
	return 0;
}
...
uint16_t Radio_Get_RDS_Data(uint32_t*rds_data) {
	uint16_t status;
	if ((Is_Radio_Atomic2||Is_Radio_Lithio) &&
		(devTEF668x_Radio_Get_RDS_DataRaw(1,&status,rds_data) == 1)) return 1;
	return 0;
}
/*--------------------------------------------------------------------
...
void Radio_CheckStation(void) {
	...
	switch(CheckIfStep) {
		...
		case 31:
		case 32:
			threshold = fm ? ((SeekSenLevel == HIGH)
						? FM_SCAN_LEVEL_HI
						: FM_SCAN_LEVEL)
					: ((SeekSenLevel == HIGH)
						? AM_SCAN_LEVEL_HI
			...
			if(Radio_Get_Level(fm) < threshold) {//exit check
				...
				} else delay(RADIO_CHECK_INTERVAL);//set for the next time
			}
			break;
